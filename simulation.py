...
class Simulation:
    ...
    def run_simulation(self, num_steps):
        for step in range(num_steps):
            acceleration, train_acceleration = self.track.get_acceleration(
                self.train.position,
                self.train.velocity,
                self.train.acceleration,
                self.time_step,
            )
            self.train.update_acceleration(train_acceleration)
            self.train.update_velocity(acceleration, self.time_step)
            self.train.update_position(self.time_step)
            print(
                f"Step {step}: Acceleration={acceleration}, Velocity={self.train.velocity}, Position={self.train.position}"
            )
